...
// ─── Classify via backend API ─────────────────────────────────────────────────
const API_URL = (import.meta.env.VITE_API_URL || "http://localhost:8000").replace(/\/$/, "");
const VALID_CATEGORIES = new Set([
  "Beauty_5", "Books_5", "Clothing_Shoes_and_Jewelry_5", "Electronics_5",
  "Home_and_Kitchen_5", "Movies_and_TV_5", "Office_Products_5", "Pet_Supplies_5",
  "Sports_and_Outdoors_5", "Toys_and_Games_5",
]);

function normalizeCategory(category: unknown) {
  const value = String(category ?? "").trim();
  return VALID_CATEGORIES.has(value) ? value : "Electronics_5";
}

async function waitForModel(onProg: (p: number, msg?: string) => void) {
  for (let attempt = 0; attempt < 180; attempt++) {
    try {
      const res = await fetch(`${API_URL}/health`);
      if (res.ok) {
        const data = await res.json();
        onProg(Math.min(20, Math.round((data.progress || 0) * 0.2)), data.message);
        if (data.model_loaded) return;
        if (data.status === "error") {
          throw new Error(data.detail || data.message || "Backend failed to load model");
        }
      }
    } catch (err) {
  ...
  throw new Error(
    `Model still loading at ${API_URL}. Keep "make start" running — first load can take 2–3 minutes.`
  );
}

async function classify(reviews: any[], onProg: (p: number, msg?: string) => void) {
  const BATCH = 8;
  const results: any[] = [];

  await waitForModel(onProg);
  onProg(20, "Classifying reviews with RoBERTa + XGBoost…");
  ...
  for (let i = 0; i < reviews.length; i += BATCH) {
    ...
    const res = await fetch(`${API_URL}/predict`, {
      ...
      body: JSON.stringify({
        reviews: batch.map(r => ({
          ...
          rating: Number.parseFloat(String(r.rating)) || 3
        }))
      })
    });
    ...
    if (!res.ok) {
      ...
      throw new Error(`Model API failed (${res.status}): ${detail}`);
    }
    ...
    data.results.forEach((pred: any, idx: number) => {
      results.push({ 
        ...batch[idx], 
        label: pred.label, 
        conf: pred.confidence,
        signal: pred.label === 0 ? "Repetitive patterns, generic language" : "Specific details, natural tone"
      });
    });

    onProg(20 + Math.min(79, Math.round(((i + BATCH) / reviews.length) * 79)));
  ...
  const run = async () => {
    ...
    try {
      const toClass = rows.map(r => ({
        text: r[cols.text] || "",
        category: cols.category ? r[cols.category] : "Electronics_5",
        rating: cols.rating ? r[cols.rating] : 3,
        raw: r,
      })).filter(r => r.text.trim());

      setMsg("Loading the classification model…");
      const results = await classify(toClass, (p, message) => {
        setProg(mode === "full" ? p * 0.7 : p);
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "60px 24px" }}>
        ...
        <div className="anim-3" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginTop:32 }}>
          {[{
            ...
            icon:"🤖", color:C.accent,
            desc:"Labels each review as fake or genuine using RoBERTa embeddings and XGBoost",
        ...
        <div className="anim-4" style={{ marginTop:48, ...sCard }}>
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"center",
            ...
            {["CSV / Excel Upload","Column Detection","Text Preprocessing",
              "RoBERTa Embeddings","XGBoost Classification","Results + Insights"].map((step,i,arr) => (
        ...
        <div className="fade-in" style={{
          ...
          <div>
            <div style={{ fontWeight:700, color: mode==="full" ? C.warn : C.accent, marginBottom:4 }}>
              {mode==="full" ? "Full Mode: Review Classification + Graph Analysis" : "Review Classification Only"}
            </div>
            <div style={{ fontSize:13, color:C.muted }}>
              {mode==="full"
                ? "All required columns detected. Reviews will be classified and checked for coordinated burst campaigns."
                : "Text column detected. Graph analysis is unavailable — missing user_id, timestamp, or product_id."}
            </div>
          </div>
        </div>
